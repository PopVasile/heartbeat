...
public class HeartBeat1 implements Runnable {

    static int self_id = 2;
    static int server_Port = 5532;
    String operation; // tip fir RECEIVER, HEARTBEAT
    static HashMap<Integer, String> processes = new HashMap<Integer, String>();
    static boolean received = false;
    static String my_ip;

    public HeartBeat1(String operation) {
    ...
    public static void main(String args[])
        ...
        Runnable receiver = new HeartBeat1("receiver");
        new Thread(receiver).start();
        Runnable heartbeat = new HeartBeat1("heartbeat");
                ...
                for (int key : processes.keySet()) {
                    if (key != self_id) {
                        ...
                        try {
                            ...
                            Socket socket = new Socket(destination_server, 5533);
                            DataOutputStream out
                                    = new DataOutputStream(socket.getOutputStream());
                            out.writeUTF("heartbeat");
                            out.writeUTF(self_id + "");
                            System.out.println("Sent HEARTBEAT1 to: " + destination_server);
                        } catch (Exception e) {
                            System.out.println("\n***\tpeer has FAILED!\t***");
                        }
                    }
